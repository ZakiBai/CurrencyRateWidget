...
class Provider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), currencyData: nil)
    }
    ...
    func createTimelineEntry(date: Date, completion: @escaping (SimpleEntry) -> ()) {
        CurrencyService
            .getCurrencyData()
            .sink { error in
                ...
                let entry = SimpleEntry(date: date, currencyData: currencyData)
                completion(entry)
            }
